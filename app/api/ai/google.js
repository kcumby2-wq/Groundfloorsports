...
const WINDOW_SECONDS = 60;
  ...
  const ipRl = await ipRateLimit(ip, 'google', { limit: 30, window: 60 });
  ...
  const rl = await rateLimit(auth.userId, 'google', { limit: MAX_REQUESTS_PER_MIN, window: WINDOW_SECONDS });
  ...
  if (typeof imageBase64 !== 'string' || imageBase64.length < 20) {
    logger.warn({ userId: auth.userId, ip, imageBase64Len: imageBase64?.length }, 'Invalid image data');
    await writeAuditLog({
      userId: auth.userId,
      ip,
      action: 'invalid_image_data',
      details: { imageBase64Len: imageBase64?.length },
    });
    return res.status(400).json({ error: 'Invalid image data' });
  }

  // Anomaly detection (usage spike)
  const anomaly = await checkAnomaly(`user:${auth.userId}`, 'google', 50);
  if (anomaly) {
  ...
  logger.info({ userId: auth.userId, ip }, 'Calling Google Vision API');
  const response = await fetch(`https://vision.googleapis.com/v1/images:annotate?key=${apiKey}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      requests: [{ image: { content: imageBase64 }, features: [{ type: 'LABEL_DETECTION' }] }],
    }),
  });
  const data = await response.json();
  logger.info({ userId: auth.userId, ip, response: data }, 'Google Vision API response');
  await writeAuditLog({
    userId: auth.userId,
    ip,
    action: 'google_vision_call',
